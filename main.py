...
def main():
    ...
    def make_block(npy_3d: np.array, npy_label: np.array, axis1):
        ...
        for i in [1, 2, 3, 5]:  # 부비동 종류
            ...
            try:
                if axis1 == 0:
                    locs = [y1, x1, y2, x2] = [locations[2].min(), locations[1].min(), locations[2].max(),
                                               locations[1].max()]
                elif axis1 == 1:
                    locs = [y1, x1, y2, x2] = [locations[2].min(), locations[0].min(), locations[2].max(),
                                               locations[0].max()]
                elif axis1 == 2:
                    locs = [y1, x1, y2, x2] = [locations[1].min(), locations[0].min(), locations[1].max(),
                                               locations[0].max()]
            except:
                locs = [300, 200, 300, 200]
            # print('locs : ',locs)
            y_mip.extend(locs)
    ...
    models = []
    for i in range(3):
        model = keras.models.load_model(model_path+'/mip_min_model%d_10_200'%(i),custom_objects={'my_loss': my_loss, 'rmse':rmse, 'my_loss2':my_loss2})
        models.append(model)
    ...
    for main_axis in range(3):
        ...
        np_x= np_x[MIDDLE_GAP:MIDDLE_GAP+HEIGHT_LEN]
    ...
    blank_numpy[a:b,c:d,e:f] = blank_target
